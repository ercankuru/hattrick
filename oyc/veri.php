...
{ echo "Lütfen Ürün Giriniz<br><a href='javascript:history.back(-1)'>Geri</a>"; exit; } 
	$sorgu = mysql_query("SELECT * FROM kimlik  ");
	$oyuncu = @mysql_fetch_row($sorgu);
	$kontrol = mysql_query("SELECT COUNT(*) FROM kimlik   ");
...
?>
  ...
  <?php 
   $tablo="SELECT * FROM form  ORDER BY tarih DESC";
...
<?php
 ...
 $tablo1="SELECT * FROM gol  ORDER BY tarih DESC";
 $tablo="SELECT * FROM tsi  ORDER BY tarih DESC";
 $sorgu=mysql_query($tablo);
...
<!-- OYUNCU YETENEK BILGILERI -->
 ...
 <?php
  $tablo="SELECT * FROM yetenek  ORDER BY tarih DESC";
  $sorgu=mysql_query($tablo);
       while ($oku = mysql_fetch_assoc($sorgu)) {
